...
correctly, saves only the valid ones, and reports accurately what happened to the rest — including
the rows it rejected deliberately.

This is the case that exercises the whole pipeline at once: file validation, parsing, per-row
normalisation, DTO rules, the duplicate SKU rule and persistence.
...
Five fail validation; five are the occurrences of two SKUs repeated within the file.
...
| 25 | HD-099 | `name` must not be empty | validation |
| 36 | RS-001 | duplicate sku (lines 2, 36) | duplicate |
| 41 | WS-001 | `name` must not be empty (whitespace only) | validation |
...
| 29 | `Robert'); DROP TABLE products;--` | created; the query is parameterised |
| 47 | `price` = `0.00` | created; zero is a valid price |
| 50 | `weight_kg` empty | created with `null`, never `0` |
| 51 | `stock` = 0 | created; the only sold-out product |
| 52 | `category` empty | created as `Uncategorized` |
...
- [x] Exactly 85 products in the catalog, spread across 18 categories
...
- [x] Lines 25 and 41 show `—` in `Name`, which is itself the reason for the failure
...
| category of `GC-025` | `Uncategorized` |
| weight of `GK-088` | `null` |
| stock of `VC-001` | `0` |
| price of `MB-001` | `0.00` |

Category split of the 85 created products:
...
`Footwear` has 2 instead of 3 because `RS-001` was rejected, and `Misc` does not appear at all:
its only row was line 41.

## Evidence

![Import summary and rows to review](assets/tc-01-import-report.png)

![Created rows and the product listing](assets/tc-01-created-rows.png)

## Defects found

| Ticket | Summary |
|---|---|
| TK-047 | Rows rejected by the duplicate SKU rule showed `—` in `Name` even though the file carries one. `rejectDuplicateSkus` was not propagating the name, unlike the other two rejection paths. |

...
| P-04.27 | La orden registra cómo se pagó | `POST /orders` con `"paymentMethod":"paypal"`, y luego con `"cash"` | El primero `201` y el detalle y el recibo dicen PayPal; el segundo `400` nombrando los métodos válidos | ✅ `create-order.dto.spec.ts` |
| P-04.24 | El carrito no muestra un total distinto del que se cobrará | Agregar al carrito, cambiar el precio en el catálogo y volver al carrito | Se adopta el precio vigente, se señala el anterior, y el total del checkout es el que registra el pedido | ✅ `cart-reconcile.test.ts` · 🔶 [TC-05](TC-05-purchase-flow.md) check 10 |
| P-04.25 | Una línea que la compra rechazaría se identifica antes de confirmar | Borrar del catálogo un producto que está en el carrito, y volver al carrito | La línea se marca no disponible y el checkout queda bloqueado hasta quitarla, en vez de fallar al confirmar sin decir cuál sobra | ✅ `cart-reconcile.test.ts` · 🔶 [TC-05](TC-05-purchase-flow.md) check 10 |
| P-04.26 | Un fallo de red no se confunde con un producto retirado | Abrir el carrito con el API caído | Se dice que no se pudo verificar el carrito; no se vacía ni se marca nada como no disponible | ✅ `cart-reconcile.test.ts` |

...
export const PRODUCT_STATUS_FILTERS: ProductStatusFilter[] = ['active', 'discontinued', 'all'];
export const DEFAULT_STATUS: ProductStatusFilter = 'active';
...
function parseStatus(raw: string | null): ProductStatusFilter {
  return PRODUCT_STATUS_FILTERS.includes(raw as ProductStatusFilter)
...
export function toProductListParams(state: IProductListState): IProductListParams {
  return {
    ...
    ...(state.status !== DEFAULT_STATUS ? { status: state.status } : {}),
    sortBy: state.sortBy,
...
export function countActiveFilters(state: IProductListState): number {
  return (
    ...
    (state.status !== DEFAULT_STATUS ? 1 : 0)
  );
}

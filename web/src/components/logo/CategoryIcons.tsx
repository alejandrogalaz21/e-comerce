...
function NeonIcon({ glyph, color, size = 64, title, className, style, ...rest }: NeonIconProps) {
  const uid = useId().replace(/:/g, '');
  const stroke: GlyphProps = {
    fill: 'none',
    stroke: color,
    strokeWidth: 2.2,
    strokeLinecap: 'round',
  ...
  const glowStroke: GlyphProps = { ...stroke, strokeWidth: 5 };
  const accentFill: GlyphProps = { fill: color, fillOpacity: 0.18 };
  const noFill: GlyphProps = { fill: 'none' };
...
export function categoryKey(category?: string | null): CategoryIconKey {
  ...
  const key = String(category)
    ...
    .toLowerCase()
    .replace(/[\s&/-]+/g, '_')
    .replace(/_+/g, '_') as CategoryIconKey;
